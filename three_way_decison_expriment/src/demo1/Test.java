...
public class Test {
	public static void main(String[] args) {
		...
		TWD t = new TWD(f);
		PosNegBndInfos pnb = t.get_once_result(f, 10);//一次分类结果
		System.out.print(pnb);
		
		DataInfo[] bndInfo = pnb.getBndInfo(); //得到边界域，继续第二次划分
...
class TWD{
	...
	public DataInfo[] getPosObjs(float[] nums, ThresholdIndex thresholdIndex){ //设计不合理----改正1------------------------------------------------------------
	...
	public DataInfo[] getNegObjs(float[] nums,ThresholdIndex thresholdIndex){
		//得到负域数组
		DataInfo[] negArray = new DataInfo[thresholdIndex.getFirstmax()]; //负域数组个数
		System.out.println(negArray.length); //---------------------------------------------
	 ...
	 * @return
	 */
	public ThresholdIndex getMaxAndNext(float[] nums){
		TWD t = new TWD(nums);
	 ...
	 * @return
	 */
	public float[] get_sub_gnnis(float[] nums){
		TWD t = new TWD(nums);
	...
	public DataInfo[] getBndObjs(float[] nums, ThresholdIndex thresholdIndex){ 
		//得到边界域数组
		DataInfo[] bndArray = new DataInfo[thresholdIndex.getSecondMax()-thresholdIndex.getFirstmax()]; //边界域数组个数
		System.out.println(bndArray.length); //---------------------------------------------
	 ...
	 * @param times 分类次数
	 */
	public PosNegBndInfos  get_once_result(float[] objArray, int len){
		float[] arrays = objArray;
		int posLen = len;
	...
	public int PosNumber(float[] nums, ThresholdIndex thresholdIndex){
	 ...
	 * @return
	 */
	public static float calc_precent(float nums[],int len, float x){
		float sum = 0;
		for(int i = 0; i < len; i++){
			sum += nums[i];
		}
	 ...
	 * @return
	 */
	public static float calc_weight(float nums[],int len,int k){ //k表示从下标0加到哪个位置
		float sum = 0;
		for(int i = 0; i < k+1; i++){ //i<len-1:表示最后一个元素不参与计算
			sum += calc_precent(nums, len, nums[i]);
		}
		return sum;
	}
	 ...
	 * @return
	 */
	public static float clac_obj_gnni(float nums[], int len){
		float sum = 0;
		if(len == 1){
			return 0;
		}else{
